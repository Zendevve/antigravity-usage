...
function getConfig() {
    ...
    return {
        ...
        adaptivePolling: config.get('adaptivePolling', true)
    };
}
async function activate(context) {
    console.log('Antigravity Usage starting...');
    // Initialize services
    quotaService = new quotaService_1.QuotaService();
    statusBarManager = new statusBar_1.StatusBarManager();
    portDetector = new portDetector_1.PortDetector();
    insightsService = new insights_1.InsightsService(context.globalState);
    cacheService = new cacheService_1.CacheService();
    // Initialize Activity Tracker for event-driven polling
    activityTracker = new activityTracker_1.ActivityTracker();
    activityTracker.setActivityCallback((state) => {
    ...
    const openDashboardCommand = vscode.commands.registerCommand('antigravity-quota.openDashboard', async () => {
        ...
        dashboardPanel_1.DashboardPanel.createOrShow(context.extensionUri, statusBarManager.getSnapshot(), cacheInfo);
    });
    context.subscriptions.push(openDashboardCommand);
    // Register Sidebar View Provider
    quotaProvider = new webviewView_1.QuotaViewProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(webviewView_1.QuotaViewProvider.viewType, quotaProvider));
    ...
    const cleanCacheCommand = vscode.commands.registerCommand('antigravity-quota.cleanCache', async () => {
        ...
        const answer = await vscode.window.showWarningMessage(`Clean ${size} MB of Antigravity cache? This will delete all Brain tasks and conversation history.`, { modal: true }, 'Clean All');
        if (answer === 'Clean All') {
    ...
    const manageCacheCommand = vscode.commands.registerCommand('antigravity-quota.manageCache', async () => {
        ...
        if (selected) {
            const confirm = await vscode.window.showWarningMessage(`Permanently delete "${selected.label}"?`, { modal: true }, 'Delete');
            if (confirm === 'Delete') {
    ...
    const flushContextCommand = vscode.commands.registerCommand('antigravity-quota.flushContext', async () => {
        ...
        const answer = await vscode.window.showWarningMessage(`Flush active context? This will clear ${preview.willDelete} items (${sizeStr}) but preserve your Brain tasks.`, { modal: true }, 'Flush Context', 'Show Details');
        if (answer === 'Show Details') {
            ...
            const proceed = await vscode.window.showInformationMessage(details, { modal: true }, 'Flush Now');
            if (proceed === 'Flush Now') {
                const result = await cacheService.flushActiveContext();
                vscode.window.showInformationMessage(`Flushed ${result.clearedConversations} conversations, ${result.clearedCodeContext} code files. Freed ${cacheService.formatBytes(result.freedBytes)}.`);
            }
        }
        else if (answer === 'Flush Context') {
            const result = await cacheService.flushActiveContext();
            vscode.window.showInformationMessage(`Context flushed! Freed ${cacheService.formatBytes(result.freedBytes)}.`);
        }
    });
...
async function detectAndPoll() {
    ...
    try {
        ...
        if (result) {
            ...
            vscode.window.showWarningMessage('Could not detect Antigravity process.', 'Retry').then(action => {
                if (action === 'Retry') {
                    detectAndPoll();
                }
            });
            ...
            if (rawSnapshot) {
                ...
                if (quotaProvider) {
                    ...
                    if (dashboardPanel_1.DashboardPanel.currentPanel) {
                        dashboardPanel_1.DashboardPanel.currentPanel.update(enrichedSnapshot, cacheInfo);
                    }
                }
            }
            else {
...
function checkQuotaWarnings(snapshot) {
    ...
    for (const model of snapshot.modelsWithInsights) {
        ...
        if (model.remainingPercent < warningThreshold) {
            if (!wasWarned) {
                ...
                vscode.window.showWarningMessage(`${model.label} is at ${model.remainingPercent}%`, 'Open Dashboard').then(action => {
